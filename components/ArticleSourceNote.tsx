...
export function ArticleSourceNote({ sources, reviewInsight }: ArticleSourceNoteProps) {
  const visibleSources = sources
    ...
    .filter((source) => {
      const isSeedNote =
        source.note?.toLowerCase().includes("bài seed") ||
        source.title?.toLowerCase().includes("dữ liệu sách và ghi chú biên tập nội bộ");
      return !isSeedNote;
    })
        ...
        <ol className="mt-4 space-y-2">
          {visibleSources.map((source) => {
            const hasDistinctDomain =
              source.domain && source.title && source.domain.toLowerCase() !== source.title.toLowerCase();
